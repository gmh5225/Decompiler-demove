...
#[derive(Clone, Debug)]
struct AliasMap {
    aliases: HashMap<usize, HashSet<usize>>,
    aliased: HashMap<usize, HashSet<usize>>,
}
...
impl AliasMap {
    ...
    fn set_alias(&mut self, dst: usize, src: usize) {
        if let Some(srcs) = self.aliases.get(&dst) {
            for src in srcs.clone() {
                self.aliased.get_mut(&src).unwrap().remove(&dst);
            }
        }
        self.aliases
            .entry(dst)
    ...
    fn set_alias_multi(&mut self, dst: usize, src_vars: &HashSet<usize>) {
        if let Some(srcs) = self.aliases.get(&dst) {
            for src in srcs.clone() {
                self.aliased.get_mut(&src).unwrap().remove(&dst);
            }
    ...
    fn destroy_alias_from(&mut self, src: usize) {
        if let Some(dsts) = self.aliased.get(&src) {
            for dst in dsts.clone() {
                self.aliases.get_mut(&dst).unwrap().remove(&src);
            }
        }
        self.aliased.remove(&src);
    }

    fn destroy_alias_to(&mut self, dst: usize) {
        if let Some(srcs) = self.aliases.get(&dst) {
            for src in srcs.clone() {
                self.aliased.get_mut(&src).unwrap().remove(&dst);
            }
        }
        self.aliases.remove(&dst);
    }

    fn merge(&self, other: &Self) -> Self {
        let mut new_alias = Self::new();
        for (dst, srcs) in self.aliases.iter() {
            if let Some(other_srcs) = other.aliases.get(&dst) {
                let intersect: HashSet<_> = srcs.intersection(other_srcs).map(|x| *x).collect();
                if !intersect.is_empty() {
                    new_alias.set_alias_multi(dst.clone(), &intersect);
                }
            }
        }
        new_alias
...
impl<'a> Clone for StacklessEvaluationContext<'a> {
    fn clone(&self) -> Self {
        ...
        Self {
            ...
            break_alias: self.break_alias.clone(),
...
impl<'a> StacklessEvaluationContext<'a> {
    ...
    fn get_pending_var(&self, idx: usize) -> Option<&VariableValueSnapshot> {
        if let Some(pending) = self.pending_variables.get(&idx) {
            ...
            for u in self.alias.aliases.get(&idx).unwrap_or(&HashSet::new()) {
                if let Some(pending) = self.pending_variables.get(u) {
                    return Some(pending);
                }
            }
            None
    ...
    pub fn get_var_with_allow_undefined(&self, idx: usize, allow_undefined: bool) -> Expr {
        ...
        if let Some(value) = self.variables.get(&idx) {
            ...
            for u in self.alias.aliases.get(&idx).unwrap_or(&HashSet::new()) {
                if let Some(value) = self.variables.get(u) {
    ...
    fn run_assignment(&mut self, idx: usize, value: Expr) -> bool {
        self.alias.destroy_alias_from(idx);
        ...
        match bytecode {
            Bytecode::Call(_, dsts, oper, srcs, _abort_action) => {
                ...
                let OperationEvaluatorResult {
                    ...
                    &srcs
                        .iter()
                        .map(|x| self.get_var_with_allow_undefined(*x, allow_undefined))
                        .collect(),
                    dst_types,
                )?;
                ...
                if cannot_keep == false
                    && results
                        ...
                        .any_variables()
                        .intersection(&HashSet::from_iter(dsts.iter().map(|x| *x)))
                        .next()
                        .is_some()
                {
                    cannot_keep = true;
    ...
    pub(crate) fn merge_branches(
        ...
        for branch in branches {
            ...
            ) in branch.variables.iter()
            {
                ...
                {
                    if *aid != *br_aid || *current_context_definition_id != *br_context_definition_id {
        ...
        // aliases is collected from tails node only
        if self_not_in_tail {
            let (first, rest) = branches.split_first().unwrap();
            self.alias = first.alias.clone();
            for b in rest {
                self.alias = self.alias.merge(&b.alias);
            }
        } else {
            for b in branches {
                self.alias = self.alias.merge(&b.alias);
        ...
        // break alias is collected from all nodes, as it's intersection of aliases of all exit (break) path
        for b in branches {
            if b.loop_entry {
                continue;
            }
            if let Some(break_alias) = &b.break_alias {
                if let Some(curr) = &self.break_alias {
                    self.break_alias = Some(curr.merge(&break_alias));
                } else {
                    self.break_alias = Some(break_alias.clone());
                }
            }
        }
    ...
    pub(crate) fn add_break_alias_point(&mut self) {
        if let Some(current_alias) = &self.break_alias {
            self.break_alias = Some(self.alias.merge(current_alias));
        } else {
            self.break_alias = Some(self.alias.clone());
        }
    }
    ...
    pub(crate) fn find_break_alias(
        &self,
        target: usize,
        possible: HashSet<usize>,
    ) -> HashSet<usize> {
        let mut result = HashSet::new();
        if let Some(break_alias) = &self.break_alias {
            if let Some(aliases) = break_alias.aliased.get(&target) {
                for alias in aliases {
                    if possible.contains(alias) {
                        result.insert(*alias);
                    }
                }
            }
            if let Some(aliases) = break_alias.aliases.get(&target) {
                for alias in aliases {
                    if possible.contains(alias) {
                        result.insert(*alias);
                    }
    ...
    pub(crate) fn set_var_alias(&mut self, v: usize, alias: HashSet<usize>) {
        self.alias.destroy_alias_to(v);
        self.alias.set_alias_multi(v, &alias);
    }

    pub(crate) fn enter_loop(&mut self) {
        self.break_alias = None;

...
pub struct VarUsage {
    ...
    pub depend_on: HashSet<usize>,
    pub dependents: HashSet<usize>,
    pub direct_assigments_chain_from: HashSet<usize>,
}
...
impl Default for VarUsage {
    fn default() -> Self {
        Self {
            ...
            depend_on: Default::default(),
            dependents: Default::default(),
            direct_assigments_chain_from: Default::default(),
        }
    }
}
...
pub struct VarUsageSnapshot<T: Default + Clone + VarUsageSnapshotWithDelta> {
    ...
    pub forward: (usize, HashMap<usize, T>),
    // snapshots usage excluding current block
    pub backward: (usize, HashMap<usize, T>),

    pub current: HashMap<usize, T::Delta>,
}
...
impl BranchMergeableVar for VarUsage {
    fn merge_branch(&self, other: &Self, base: &Option<&Self>) -> Option<Self> {
        ...
        Some(Self {
            ...
            depend_on: self.depend_on.union(&other.depend_on).copied().collect(),
            dependents: self.dependents.union(&other.dependents).copied().collect(),
            direct_assigments_chain_from: self
                .direct_assigments_chain_from
                .union(&other.direct_assigments_chain_from)
                .copied()
                .collect(),
        })
    }
    ...
    fn run_unit(
        ...
        *config.t.borrow_mut() = 0;
        ...
        unit_block
            ...
            .forward = (*config.t.borrow(), state.snapshot());
    ...
    fn run_unit(
        ...
        *config.t.borrow_mut() = 0;
        ...
        unit_block
            .meta_mut()
            .get_or_default::<VarUsageSnapshot<VarUsage>>()
            .backward = (*config.t.borrow(), state.snapshot());
        ...
        unit_block
            .meta_mut()
            .get_or_default::<VarUsageSnapshot<VarUsage>>()
            .current = state.delta(original_state.as_ref());
    ...
    fn run_basicblock(
        ...
        for inst in basic_block.inner_mut().content.code.iter_mut() {
            *config.t.borrow_mut() += 1;
            ...
            inst.meta_mut()
                .get_or_default::<VarUsageSnapshot<VarUsage>>()
                .current = state.delta(prev_state.as_ref());
        }
...
fn update_state(
    ...
    match &inst.bytecode {
        Assign(_, dst, src, _) => {
            ...
            if running_forward {
                svar.dependents.insert(*dst);
            }
            let dvar = state.get_or_default(dst);
            dvar.add_write(t, state_time_id);
            if running_forward {
                dvar.depend_on.insert(*src);
                dvar.direct_assigments_chain_from.insert(*src);
            }
        }
        ...
        Call(_, dsts, op, srcs, _) => {
            ...
            for &src in srcs {
                ...
                svar.add_read(t, state_time_id);
                if running_forward {
                    svar.dependents.extend(dsts.iter().copied());
                    for dst in dsts {
                        let dvar = state.get_or_default(dst);
                        dvar.depend_on.insert(src);
                    }

...
impl<'a> SourceGen<'a> {
    ...
    fn can_ignore_variable_assigment(
        ...
        let final_var_usage = &s_ctx.block_final_var_usage.forward.1.get(&v);
        ...
        let forward_var_usage = &var_usage.forward.1.get(&v);
    ...
    fn check_need_declare(
        ...
        {
            let forward_state = &node_var_usage.forward.1;
            if deps.iter().all(|x| {
        ...
        {
            let forward_state = &node_var_usage.forward.1;
            let final_var_usage = &s_ctx.block_final_var_usage.forward.1;
    ...
    fn visit_codeunit(
        ...
        if !current.is_terminated_in_loop() {
            if !future_need_vars.is_empty() {
                let mut variables = future_need_vars.iter().map(|x| *x).collect::<Vec<_>>();
                variables.sort();
    ...
    fn visit_hyperblock(
        ...
        match block.inner() {
            ...
            HyperBlock::IfElseBlocks { if_unit, else_unit } => {
                ...
                let t_vars = find_need_propagate_inner_defining_variables(if_unit, evaluation_ctx);
                let f_vars = find_need_propagate_inner_defining_variables(if_unit, evaluation_ctx);

                if !is_same_hashset(&t_vars, &f_vars) {
                    return Err(anyhow::anyhow!(
                        "if else branch defining variables mismatch"
                    ));
                }
                ...
                for &v in &t_vars {
                    t_s_ctx.need_propagate(v);
                    f_s_ctx.need_propagate(v);
                }
                ...
                for v in evaluation_ctx.merge_branches(&vec![&t_ctx, &f_ctx], true) {
                    let has_future_usage = if let Some(usage) = var_usage.forward.1.get(&v) {
                ...
                let mut result_variables = t_vars.iter().map(|x| *x).collect::<Vec<_>>();
            ...
            HyperBlock::WhileBlocks {
                ...
                for v in evaluation_ctx.merge_branches(&vec![&i_ctx], false) {
                    let has_future_usage = if let Some(usage) = inner_var_usage.forward.1.get(&v) {
                        usage.read_cnt + usage.write_cnt > 0
                    } else {
                        false
                    };
                    ...
                    if evaluation_ctx.defined(v) {
                        ...
                        if unconditional {
                            evaluation_ctx.set_var_alias(
                                v,
                                i_ctx.find_break_alias(v, evaluation_ctx.get_vars()),
                            );
                        } else {
                            panic!("conditional while loop should not have leaked variables");
                        }
                    }
                }
    ...
    fn visit_basicblock(
        ...
        while let Some(bytecode) = iter.next() {
            ...
            let dst_tmps = match &bytecode.bytecode {
                ...
                Call(_, dsts, _, _, _) => dsts.iter().map(|x| *x).collect::<Vec<_>>(),
        ...
        match block.next {
            ...
            | Terminator::While { .. } => {}
            Terminator::Break { .. } => {
                evaluation_ctx.add_break_alias_point();
...
fn is_same_hashset(t_vars: &HashSet<usize>, f_vars: &HashSet<usize>) -> bool {
    f_vars.len() == t_vars.len() && t_vars.is_subset(f_vars)
}
...
fn find_need_propagate_inner_defining_variables(
    ...
    let self_var_access = &snapshot.current;
    let future_vars_usage = &snapshot.forward.1;

...
pub fn decompile(
    ...
    algo::loop_reconstruction::loop_reconstruction(&mut blocks)?;
    ...
    let mut program = build_program(blocks.iter(), false)?;
    ...
    for block in program.blocks.iter().map(to_inner) {
        match block {
            ...
            HyperBlock::WhileBlocks {
                ...
                println!(
                    "{}While unconditional={}, start_label={}",
                    prefix, unconditional, start_label
                );
    ...
    while let Some(block) = iter.next() {
        ...
        match block {
            ...
            HyperBlock::IfElseBlocks { if_unit, else_unit } => {
                ...
                cleanup_jumps(&mut if_unit.inner_mut().blocks, &labels, &loop_labels);
                cleanup_jumps(&mut else_unit.inner_mut().blocks, &labels, &loop_labels);
            }
            ...
            HyperBlock::WhileBlocks {
                ...
                cleanup_jumps(&mut outer.inner_mut().blocks, &labels, &loop_labels);
            }
        }
    }
}
    ...
    loop {
        ...
        if let Some(node) = iter.next() {
            if let (true, Some(exit)) = (
                ...
                let paths = follow_loop_boundaries(&mut iter, node.idx, node.idx, exit, true)?;
                ...
                match node.next {
                    ...
                    Terminator::IfElse {
                        ...
                        chaining_blocks.push(node.clone().with_metadata());
                        flush(&mut chaining_blocks, &mut p);
                        let paths = follow_ifelse_boundaries(&mut iter, if_block, else_block)?;
                        p.inner_mut().blocks.push(paths);
                    }
...
fn follow_loop_boundaries(
    ...
    let inner_program = build_program(inner_nodes.iter(), unconditional)?;
    let outer_program = build_program(outer_nodes.iter(), false)?;
...
fn follow_ifelse_boundaries(
    ...
    let true_program = build_program(true_nodes.iter(), false)?;
    let false_program = build_program(false_nodes.iter(), false)?;

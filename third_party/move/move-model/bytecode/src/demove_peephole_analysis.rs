...
// Peephole optimization for Verichains decompiler. Oct 2023, NAQ
            ...
            if let Bytecode::Call(_, _, oper, srcs, _) = insn {
                ...
                if !block_offsets.contains(&offset) && matches!(oper, Operation::Destroy) {
                    ...
                    if let Some(last_insn) = new_code.last_mut() {
                        match last_insn {
                            // pattern: Load(AttrId(8), 8, U64(1));  Call(AttrId(9), [], Destroy, [8], None)
                            ...
                            // Call(AttrId(6), [7], Add, [5, 6], None); Call(AttrId(7), [], Destroy, [7], None)
